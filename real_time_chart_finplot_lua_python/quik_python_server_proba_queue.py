...
class DeltaBar:
    ...
    def run(self, parse):
        ...
        if abs(self.df.iloc[-1]['delta']) >= 500:
            ...
            # Для исключения на быстром рынке повторений баров с одним временем
            # конструкция "parse[8][0:-1]" убирает знак конца строки в parse[8]
...
if __name__ == '__main__':
    ...
    delta_bar = DeltaBar()  # Создаем экземпляр класса DeltaBar
    q = Queue()
